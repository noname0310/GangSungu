...
public struct LexEnumerator : IEnumerator<Token>
{
    ...
    private static Token? Convert(LowToken lowToken, Pos low, in Source source)
    {
        ...
        else if (lowToken.Kind.Enum == LowTokenKindEnum.Literal)
        {
            ...
            switch (literal.Enum)
            {
                case Low.Tokens.TokenLiteralKindEnum.Number:
    ...
    public static void CheckLowToken(in LowToken token, Span span, in Source source) {
        match token.kind() {
